...
var defaultOpsfiles = []string{opsfileChangeName, opsfileChangeVersion}
...
func deployPcap(baseManifestVars baseManifestVars, customOpsfiles []string, customVars map[string]interface{}, expectSuccess bool) {
	manifestVars := buildManifestVars(baseManifestVars, customVars)
	opsfiles := append(defaultOpsfiles, customOpsfiles...)
	cmd, _ := deployBaseManifestCmd(baseManifestVars.deploymentName, opsfiles, manifestVars)
